<template>
  <view class="tabs">
    ...
    <view class="btn-wrapper">
      <button class="oc-btn-wrapper-button" plain type="primary" @click="checkOrigin()">原点检测</button>
    </view>
    <!-- // 保存按钮 -->
    <view class="btn-wrapper">
      <button class="btn-wrapper-button" plain type="primary" @click="submit(pageId)" :disabled="userModel == 1">保存</button>
    </view>
  </view>
</template>
...
export default {
  ...
  created() {
    ...
    this.queryParams.mainModel = uni.getStorageSync("mainModel");
    this.queryParams.subModel = uni.getStorageSync("subModel");
    uni.showLoading({
      title: "数据加载中",
    });
